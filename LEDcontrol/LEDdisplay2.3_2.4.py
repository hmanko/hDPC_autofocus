...
class TiffWriterThread(threading.Thread):
    def __init__(self, frame_queue, base_filename):
        ...
        self.writer = None
        self.current_size = 0
        self.timestamp = self.timestamp = datetime.now().strftime("%Hh%M%S")

    def _open_new_file(self):
        if self.writer:
            self.writer.close()

        filename = file_name_check(f"{self.base_filename}_{self.timestamp}_part_{self.file_index:03d}.tif")
        self.writer = TiffWriter(filename, bigtiff=True)
        self.current_size = 0
        self.file_index += 1
        print(f"[Writer] Opened {filename}")
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                if self.current_size > MAX_FILE_SIZE:
                    self._open_new_file()
                self.writer.write(frame, contiguous=True)
                self.current_size += frame.nbytes
...
class AcuqisitionThread(QThread):
    ...
    def run(self):
        ...
        try:
                ...
                with mmc_lock:
                    if mmc.getRemainingImageCount() > 0:
                        ...
                        if image_count % 5 == 0:
                            self.frame_received.emit(image)
                        try:
                            self.frame_queue.put(image)#put(image.astype(np.float16))  
                        except queue.Full:
            ...
            self.running = False
            self.frame_queue.put(None)   ### was commented
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.illumination_pattern_change('2 LED')
